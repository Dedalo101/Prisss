/** Prisss visuals — deferred for PageSpeed */
(function () {
  'use strict';
  const IS_MOBILE = window.innerWidth < 768;
  const IS_TOUCH = window.matchMedia('(hover: none), (pointer: coarse)').matches;
  const REDUCE = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (REDUCE) { document.body.classList.add('visuals-static'); return; }
  const start = () => {
...
const RINGS = IS_MOBILE ? 36 : 90;
const GROOVE_STEPS = IS_MOBILE ? 72 : 160;
const SPIRAL_STEPS = IS_MOBILE ? 220 : 700;
const ATTRACTOR_STEPS = IS_MOBILE ? 100 : 380;
const PCOUNT = IS_MOBILE ? 50 : 240;
const DRAW_LINKS = !IS_MOBILE;
let grooves = [];
...
function drawGrooves(t) {
  ...
  grooves.forEach(g => {
    ...
    ctx.beginPath();
    for (let s = 0; s <= GROOVE_STEPS; s++) {
      const a = (s/GROOVE_STEPS) * Math.PI * 2;
...
frame();
  ...
  const run = () => (window.requestIdleCallback || (cb => setTimeout(cb, 40)))(start, { timeout: 2500 });
